...
public class MovieAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        mHolder.txt_actor.setText( this.getHTML(movieVOList.get(position).getActor()) );
    ...
    // 문자열에HTML Tag가 들어있는 경우 해당문자열에 HTML 효과를 적용하기 위해
    // 변환 method를 생성
    private Spanned getHTML(String strText){
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
            spText = Html.fromHtml(strText,Html.FROM_HTML_MODE_COMPACT);
        }else {
