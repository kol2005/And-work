...
/*
View class 이야기
안드로이드에서 눈에 보이는 모든것 (Layout, Button, Textview, TextEdit)은
모두 View 클래스를 상속받아서 만들어진 component 들이다
어떤 이벤트나 액션등을 지정할때 기존의 클래스나 인터페이스를 상속 또는 implements 해야 하는데
...
public class MyViewClass implements View.OnClickListener {
    ...
    btn1을 클릭하면 클릭된 Button의 모든 요소가 view인 v 객체변수에 담겨
    onClick() method로 전달된다
     */
    @Override
    public void onClick(View v) {
        ...
        누가, 호출했는가를 알고 싶을때 v.getId() method를
        사용하면 view(comp)의 id값을 얻어올수 있다
         */
        String msg = "방가방가";
        if(v.getId() == R.id.btn1) {
            ...
            /*
            이벤트가 발생한 view(comp)로 부터 어떤 값을 얻어오고자 하면
